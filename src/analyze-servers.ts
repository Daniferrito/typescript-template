...
export async function main(ns: NS): Promise<void> {
  ...
  for (const server of servers) {
    ...
    // ns.print(`Server: ${server}, Security: ${security}/${minSecurity}, Money: ${ns.formatNumber(moneyAvailable)}/${ns.formatNumber(moneyMax)}`)
    const cores = ns.getServer(server).cpuCores ?? 1
    const ram = ns.getServerMaxRam(server)
    ns.print(`Server: ${server}, Cores: ${cores}, RAM: ${ns.formatRam(ram)}`)
  }
}